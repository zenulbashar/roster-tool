...
- [x] M31 — Drag-and-drop roster builder + per-person shift times & breaks: the builder's staff×day grid is now interactive (`RosterBoard`, @dnd-kit) — drag a shift chip across days/staff to move it (atomic `moveAssignment`; the target shift is matched by type, with a chooser when several match and a create-and-move offer when none exists that day), drag to/from the **Open shifts** row to release/assign, and accept/clear suggestions inline. Chips are **colour-coded per employee** (the existing `avatarColor`, with an Employee/Type toggle) and carry a proportional time-span bar. Tapping a chip opens a **schedule editor** (drag handles + accessible steppers, 15-min snapping) that gives THAT PERSON their own start/end and an unpaid 30/60-min break on the shift — stored as a nullable per-assignment override (additive migration `0028`; null = the shift's times, so nothing changes until the owner shapes a shift). Resolved times flow to the public roster and publish emails; "Draft from last week" copies each person's shaped schedule. **Rostered times/breaks are planning aids only — paid hours still come exclusively from clocked timesheets** (CSV/report/Xero untouched). The tap-to-assign editor below the grid remains as the keyboard/mobile-accessible fallback. Pure logic in `src/lib/roster-schedule.ts` + `src/lib/roster-drag.ts`. Plan: `docs/drag-drop-roster-plan.md`.
